	...
	$.widget( "ui.tapestryFieldEventManager", {
		...
		getControlGroup : function() {
			if ( !this.options.controlGroup ) {
				var controlGroup = $("#"+this.options.id).parents('.control-group');
				if ( controlGroup.length > 0 ) {
					this.options.controlGroup = controlGroup;
				}
			}
			return this.options.controlGroup;
		},
		...
		removeDecorations : function() {
			var field = this.element;
			if ( this.getControlGroup() ) {
				this.getControlGroup().removeClass("error");
			} else {
		...
		showValidationMessage : function(message) {
			...
			if ( this.getControlGroup() ) {
				this.getControlGroup().addClass("error");
			} else {
			...
			if($("#"+id).size()==0) {//if the errorpopup isn't on the page yet, we create it
				if ( this.getControlGroup() ) {
					field.after("<span id='"+field.attr('id')+":errorpopup' class='help-inline'/>");
				} else {
